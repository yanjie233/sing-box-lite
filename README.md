...
- 域名（域名证书模式）
- ACME 账户邮箱
- HTTP 验证端口，可自定义，默认 `80`；也可通过环境变量 `ACME_HTTP_PORT` 预填

验证端口必须能从公网访问；如果前面有端口转发，请把公网验证端口转发到脚本设置的本地端口。交互安装时直接输入端口即可，回车使用当前值或默认值 `80`。证书申请失败时脚本会停止，不会用不完整证书启动 Hysteria2。
